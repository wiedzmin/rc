	...
	def app_default(self, c):
		...
		if f.extension is not None:
			if f.extension in ('pdf', ):
				c.flags += 'd'
				return self.either(c, 'apvlv', 'okular', 'zathura', 'evince')
			if f.extension == 'djvu':
				c.flags += 'd'
				return self.either(c, 'apvlv', 'okular', 'evince')
			if f.extension == 'txt':
				c.flags += 'd'
				return self.either(c, 'fbreader')
			if f.extension in ('xml', ):
				return self.either(c, 'editor')
			if f.extension in ('html', 'htm', 'xhtml'):
				c.flags += 'd'
				return self.either(c, 'conkeror', 'firefox', 'opera', 'jumanji',
						'luakit', 'elinks', 'lynx')
			if f.extension == 'swf':
		...
		if f.image:
			c.flags += 'd'
			return self.either(c, 'gwenview', 'feh', 'eog', 'mirage')
